  ...
  useEffect(() => {
    const handleProgressSubmit = async () => {
        ...
        for (const reportType of reportTypes) {
          ...
          promises.push(
            dispatch(fetchUserAnalytics({ progressObj: reviewdata }))
              .then((response) => {
                ...
                if (totalSummary.length > 0 && reportType == "annotation") {
                  projectResults[projectType][reportType] = {
                    ...
                    avgAnnotationTime:
                      totalSummary[0]?.["Avg Annotation Time (sec)"] || 0,
                  };
              ...
              .catch((error) => {
                console.error(
                  `Error loading ${projectType} - ${reportType}:`,
                  error,
                );
              })
              .finally(() => {
      ...
      const formattedData = projectTypes.map((projectType) => ({
        ...
        avgAnnotationTime:
          projectResults[projectType].annotation.avgAnnotationTime || 0,
      }));
        ...
        <div style={{ width: "100%", height: "360px" }}>
          <ResponsiveContainer>
            ...
            >
              ...
              <XAxis
                dataKey="name"
                tick={{ fontSize: 10, textAnchor: "middle", angle: -5 }}
                interval={0}
              />
              <YAxis />
