...
export const fetchwsMetaAnalyticsData = createAsyncThunk(
  'getwsMetaAnalyticsData/fetchwsMetaAnalyticsData',
  async ({wsID,project_type_filter,progressObj}) => {
    let endpoint ;
    const body = progressObj
    project_type_filter=='AllTypes'?
    endpoint = `${ENDPOINTS.getWorkspaces}${wsID}/cumulative_tasks_count_all/?metainfo=true`
    :
    endpoint = `${ENDPOINTS.getWorkspaces}${wsID}/cumulative_tasks_count_all/?metainfo=true&project_type_filter=${project_type_filter}`
    const params = fetchParams(endpoint);
    return fetch(params.url, params.options)
        .then(response => response.json())
  }
);
const getwsMetaAnalyticsData = createSlice({
  name: 'getwsMetaAnalyticsData',
  initialState,
  reducers: {},
  extraReducers: (builder) => {
    builder
      .addCase(fetchwsMetaAnalyticsData.pending, (state) => {
        state.status = 'loading';
      })
      .addCase(fetchwsMetaAnalyticsData.fulfilled, (state, action) => {
...
export default getwsMetaAnalyticsData.reducer;