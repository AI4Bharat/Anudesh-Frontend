...
export default function PerformanceAnalytics() {
  ...
  const handleAnalyticsData = () => {
    ...
    for (let i of performanceAnalyticsTasksData) {
      ...
      const item = i?.data?.[0];   // ✅ safe access
      ...
      if (metaInfo && AudioTypes.includes(selectedType)) {
        let t = item?.periodical_aud_duration;
      ...
      // 📊 META INFO CASE (WORD / PROMPT)
      else if (metaInfo) {
        const value =
          item?.periodical_word_count ??   // old backend
          item?.total_prompt_words ??      // your new change
          0;
      ...
      // 📦 NORMAL TASK COUNT
        ...
        entries.push(item?.periodical_tasks_count || 0);
      }
    }
